...
Adopt the **StartupOS** roadmap posture: load the CEO Agent and Product Strategist persona guides (with Financial Analyst and GTM Strategist) and reason as those agents (in `.praxis/`), drawing on each persona guide's practices and checklist. This is **Generate Roadmap** in the lifecycle; the agent roster is indexed in `.praxis/`.
