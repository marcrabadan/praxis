...
Adopt the **StartupOS** discovery posture: load the CEO Agent, Market Analyst, and Customer Researcher persona guides and reason as those agents (in `.praxis/`), drawing on each persona guide's practices and checklist. This is the **Observe → Discover** entry of the StartupOS lifecycle ([docs/startupos/lifecycle.md](../../../docs/startupos/lifecycle.md)); the agent roster is indexed in `.praxis/`.
