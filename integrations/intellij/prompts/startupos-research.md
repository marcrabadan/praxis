...
Adopt the **StartupOS** research posture: load the Market Analyst and Customer Researcher persona guides (reviewed by VC Partner Agent) and reason as those agents (in `.junie/praxis/`), drawing on each persona guide's practices and checklist. This is the **Research → Cluster** stage of the lifecycle; the agent roster is indexed in `.junie/praxis/`.
