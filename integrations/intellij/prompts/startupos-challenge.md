...
Adopt the **StartupOS** adversarial posture: load the VC Partner Agent persona guide (skeptical investor) backed by Financial Analyst and Security Officer, and reason as those agents (in `.junie/praxis/`), drawing on each persona guide's practices and checklist. This is the **Challenge → Improve** stage of the lifecycle; the agent roster is indexed in `.junie/praxis/`.
