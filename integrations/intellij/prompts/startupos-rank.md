...
Adopt the **StartupOS** ranking posture: load the VC Partner Agent and CEO Agent persona guides (advised by Financial Analyst and Product Strategist) and reason as those agents (in `.junie/praxis/`), drawing on each persona guide's practices and checklist. This is the **Score → Rank** stage of the lifecycle; the agent roster is indexed in `.junie/praxis/`.
