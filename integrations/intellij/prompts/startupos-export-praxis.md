...
Adopt the **StartupOS** handoff posture: load the CEO Agent and CTO Agent persona guides to assemble the package; **the human approves the export** (draw on each persona guide's practices and checklist). This is **Export to Praxis** — the **final hard human gate** in the lifecycle. The agent roster is indexed in `.junie/praxis/`; the handoff detail is in [docs/startupos/praxis-integration.md](../../../docs/startupos/praxis-integration.md).
