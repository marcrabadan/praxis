...
Adopt the **StartupOS** ranking posture: load the VC Partner Agent and CEO Agent persona guides (advised by Financial Analyst and Product Strategist) and reason as those agents (the `praxis-startupos-*` rules), drawing on each persona guide's practices and checklist. This is the **Score → Rank** stage of the lifecycle; the agent roster is indexed in the `praxis-startupos-*` rules.
...
- The candidate set in `memory/startupos/ideas/` (filtered by the scope you give it if given).
