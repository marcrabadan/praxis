...
Adopt the **StartupOS** selection posture: load the CEO Agent persona guide to facilitate, but **the human decides** (draw on the persona guide's practices and checklist). This is the **Select** stage — the **first hard human gate** in the lifecycle; the agent roster is indexed in the `praxis-startupos-*` rules.
