...
StartupOS commands follow Praxis command conventions: each is a flat Claude Code-native command file [`.claude/commands/startupos-<name>.md`](../../.claude/commands/) with `description` + `argument-hint` frontmatter, discovered automatically and invoked as `/startupos-<name>`.

> **Naming.** Claude Code derives a `.claude/commands/` command name from the **file name** (a subdirectory is not part of the command name — it only shows in the description). So the commands are flat files prefixed `startupos-` (e.g. `startupos-discover.md` → `/startupos-discover`), which keeps the StartupOS grouping in the command name and registers identically across the Claude Code CLI, desktop, and web. Installed as the praxis plugin they namespace to `/praxis:startupos-<name>`. Existing flat Praxis commands (`/architect`, `/new-feature`, …) are untouched.
