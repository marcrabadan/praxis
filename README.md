...
Optionally, the **[StartupOS](docs/startupos/README.md)** module sits *in front of* all this: an AI-native front door for discovering, validating, and designing a business idea *before* Praxis builds it — ending in a human-approved handoff to the Praxis lifecycle. It's one optional intake, not the point of the repo. See [StartupOS — the pre-build front door](#startupos--the-pre-build-front-door-optional-module).
...
| `devops-engineer` | DevOps Engineer | CI/CD, IaC, containers/K8s, deployment strategies, observability/SLOs, incident response, DORA, plus an optional Terraform multi-cloud deploy phase (K8s/AWS/GCP/Azure) via MCP. |
...
Praxis splits its full feature lifecycle into two halves — a **Product Developer Life Cycle** (the *what & why*: discovery → research → product definition → spec → experience) and a **Software Developer Life Cycle** (the *how*: plan → tasks → build → verify → release → deploy). Each phase is owned by a role and every gate is human-in-the-loop. The final **deploy** phase is optional and config-gated: it runs only when `.praxis/config.json` declares a deploy target.
...
- **The full feature lifecycle.** In harness mode, `/new-feature` runs `discovery → research → product-definition → spec → experience → plan → tasks → build → verify → release-candidate → release → deploy` and writes the durable, typed artifacts under `projects/<project>/specs/<spec>/` — `discovery/`, `research/` (report + evidence log + alternatives), the PO-owned `product-definition` (MVP/scope/metrics), `spec.md`, optional `experience/` contracts, `plans/implementation-plan.md`, `tasks/tasks.md`, `decisions/`, `reports/verify/`, and `reports/release/` (with `release-candidate` separating *proven-correct* from *decided-to-ship*, and an optional `deploy-report.md`) — instead of leaving the plan only in chat. **Research precedes the spec.** The terminal **`deploy` step is optional and config-gated** — Terraform + multi-cloud (K8s/AWS/GCP/Azure) via MCP, with pre-apply guardrails, supply-chain checks and an SLO-gated rollout; with no declared target it records a skip. Doctrine: [`systems/feature-development/artifact-model.md`](systems/feature-development/artifact-model.md) and [`.claude/skills/devops-engineer/references/deploy.md`](.claude/skills/devops-engineer/references/deploy.md).
...
#    /new-feature "<idea>"   → writes projects/checkout/specs/<slug>/ (discovery → … → release → deploy)
...
## StartupOS — the pre-build front door (optional module)

Praxis answers **"how do we build the product correctly?"**. **[StartupOS](docs/startupos/README.md)** is an optional module that answers the question *before* it: **"what should we build, why, for whom, and how do we turn it into a real company?"**

StartupOS is an AI-native operating system for **discovering, validating, designing, and preparing** startup/business ideas — and then handing a validated, **Praxis-ready project** to the existing Praxis lifecycle. It is layered *on top of* Praxis, reuses the same conventions (Claude Code-native commands, markdown templates, a memory model, human-in-the-loop gates), and **never builds product** — its terminal action is the handoff. Praxis is the engine; StartupOS is one optional intake on the front of it, not its main purpose.
