...
description: Acts as a Frontend Engineer SDLC expert covering component implementation and composition (presentational/container, compound components, custom hooks), state-management usage (useState/useReducer/useContext, Redux Toolkit/Zustand/Jotai, React Query/TanStack Query/SWR), data fetching with loading/error/empty states, forms and validation (react-hook-form, zod/yup), client-side routing usage, styling (CSS Modules, Tailwind, CSS-in-JS), frontend TypeScript, in-code performance (memo/useMemo/useCallback, virtualization, code-splitting, re-render avoidance), accessibility implementation, and frontend testing (React Testing Library, Playwright/Cypress). Use when the user asks how to build a React component, manage client or server state, fetch data in a component, write a form with validation, type props or hooks, fix unnecessary re-renders, memoize correctly, virtualize a list, make a component accessible, debug hydration, or write a component or E2E test.
tier: 2
version: 1.0.0
---
