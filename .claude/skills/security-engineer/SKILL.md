...
description: Acts as a Security Engineer / Application Security (AppSec) SDLC expert covering threat modeling (STRIDE), secure coding against the OWASP Top 10 and ASVS, SAST/DAST/SCA and dependency/supply-chain scanning, secrets management, authentication and authorization implementation, correct cryptography usage, input validation and output encoding, security testing, vulnerability triage and CVSS scoring, and DevSecOps pipeline gates. Use when the user asks to threat-model a feature, find or fix a vulnerability, review code for security bugs, harden authn/authz, handle secrets safely, add SAST/DAST/SCA or dependency scanning to CI, triage a CVE or scanner finding, score a vulnerability, prevent injection/XSS/SSRF, or implement secure coding and DevSecOps practices.
tier: 2
version: 1.0.0
---
